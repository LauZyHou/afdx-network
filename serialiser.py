...
def generateOutput(net, filePath):
    ...
    for flow in net.flows.values():
        ...
        for target in flow.targets.values():
            target_d = ET.SubElement(
                flow_d, "target", name=target.name, value=str(target.delay))
            target_j = ET.SubElement(
                flow_j, "target", name=target.name, value=str(target.jitter))
    ...
    for s in net.switches.values():
        ...
        for link in s.links:
            port = ET.SubElement(switch, "port", num=str(
                0), backlog=str(0), delay=str(0))
        total = ET.SubElement(switch, "total", backlog=str(
            0), buffer=str(0), percent=str(0))
    ...
    for link in net.links.values():
        ...
        loadDirect = ET.SubElement(edge, 'usage', type="direct", value="{:.4e}".format(
            link.loadDirect*link.trans_cap), percent=loadStrD)
        loadStrR = str(round(link.loadReverse*100, 1))+"%"
        loadReverse = ET.SubElement(edge, 'usage', type="reverse", value="{:.4e}".format(
            link.loadReverse*link.trans_cap), percent=loadStrR)
